...
const imprimirBotonesPagoCarrito = (espectaculos, carrito) => {
  ...
  $("#boton-pago")
    .hide()
    .append(
      `<div class="container pagos-container">
    ...
    .fadeIn(500);
  $("#idButtonPagoCarrito").click(() => {
    finalizarCompra(espectaculos, carrito);
  });
  $("#idButtonVaciarCarrito").click(() => {
    vaciarCarrito(espectaculos, carrito);
  });
};
...
const actualizarCantidadDisponibleEnTarjeta = (espectaculo) => {
  ...
  if (espectaculo.obtenerStock === 0) {
    ...
    $(`#${idCantidadTarjetaEspectaculo} input`)
      .addClass("disabled")
      .prop("disabled", true); // Deshabilita number box
  }
};
...
const agregarAlCarrito = (espectaculo, carrito) => {
  ...
  if ($(`#${idTarjetaEspectaculo}`)) {
    ...
    if (
      cantEntradas &&
      cantEntradas > 0 &&
      cantEntradas <= espectaculo.obtenerStock
    ) {
      // Validar que la cantidad sea menor al stock de entradas disponible
      ...
      if (pos >= 0) {
        // Si existe le sumo la cantidad, sino lo creo y agrego al carrito
      ...
      mostrarMensajeEnModal(
        `Se agregaron ${cantEntradas} entradas para "${espectaculo.nombre}" al carrito`
      );
    } else if (cantEntradas === 0) {
      mostrarMensajeEnModal(
        "Por favor seleccione la cantidad de entradas",
        true
      );
    } else if (cantEntradas > espectaculo.obtenerStock) {
      mostrarMensajeEnModal(
        `Superó el número de entradas disponibles. Para "${espectaculo.nombre}" quedan ${espectaculo.obtenerStock} entradas.`,
        true
      );
    }
  }
...
const eliminarDelCarrito = (espectaculos, carrito, itemCarrito) => {
  ...
  const posEspectaculo = espectaculos.findIndex(
    (elemento) => elemento.id === itemCarrito.id
  ); // Busco posición del elemento en la lista de espectaculos
  ...
  mostrarMensajeEnModal(
    `${itemCarrito.nombre} fue eliminado del carrito`,
    true
  );
  if (carrito.length === 0) {
...
const buscador = (espectaculos, carrito, busqueda) => {
  ...
  if (busqueda.trim() === "") {
    ...
    const listaResultados = espectaculos.filter((espectaculo) =>
      espectaculo.nombre
        .toLowerCase()
...
const convertirEspectaculosStorageEIniciarPagina = (
  listaStorage,
  listaConvertida,
  carrito
) => {
...
const inicializarDatosYPagina = (
  espectaculosStorageConvertidos,
  listaDeCarrito
) => {
  ...
  if (!localStorage.key(KEY_ESPECTACULOS)) {
    ...
    $.getJSON(URLJSON, (listaDeEspectaculos, estado) => {
      if (estado === "success") {
        ...
        localStorage.setItem(
          KEY_ESPECTACULOS,
          JSON.stringify(listaDeEspectaculos)
        );
        convertirEspectaculosStorageEIniciarPagina(
          listaDeEspectaculos,
          espectaculosStorageConvertidos,
          listaDeCarrito
    ...
    const espectaculosStorage = JSON.parse(
      localStorage.getItem(KEY_ESPECTACULOS)
